...
export function useWebRTC() {
  ...
  const getLocalStream = useCallback(async () => {
    ...
    try {
      ...
      alert('Mikrofon ruxsati berilmadi. Iltimos sozlamalardan mikrofonga ruxsat bering.');
      throw error;
    }
  }, []);
  ...
  const addPendingCandidates = useCallback(async (pc: RTCPeerConnection) => {
    if (pendingCandidates.current.length > 0 && pc.remoteDescription) {
      console.log('WebRTC: Adding', pendingCandidates.current.length, 'pending candidates');
