...
export default function PartnersPage() {
  ...
  const [tab, setTab] = useState<'partners' | 'search' | 'requests'>('partners');
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <header className="bg-slate-800 px-4 py-3 flex items-center justify-between">
        <button onClick={() => router.push('/dashboard')} className="text-slate-400">
          ← Orqaga
        </button>
      ...
      <div className="flex border-b border-slate-700">
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium ${
            tab === 'partners' ? 'text-violet-400 border-b-2 border-violet-400' : 'text-slate-400'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium ${
            tab === 'search' ? 'text-violet-400 border-b-2 border-violet-400' : 'text-slate-400'
          }`}
        >
          Qidirish
        </button>
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium relative ${
            tab === 'requests' ? 'text-violet-400 border-b-2 border-violet-400' : 'text-slate-400'
          }`}
        >
          So'rovlar
          {requests.length > 0 && (
          ...
          <div className="space-y-3">
            {partners.length === 0 ? (
              ...
              partners.map((partner) => (
                <div key={partner.id} className="bg-slate-800 rounded-xl p-4 flex items-center justify-between">
          ...
          <div>
            ...
            <div className="space-y-3">
              {searchResults.map((user) => (
                <div key={user.id} className="bg-slate-800 rounded-xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${
