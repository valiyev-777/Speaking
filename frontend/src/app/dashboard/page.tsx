...
export default function DashboardPage() {
  ...
  const { user, isAuthenticated, queueStatus, currentMatch, isInSession, logout } = useStore();
  const { isConnected, joinQueue, leaveQueue } = useWebSocket();
  
  const [selectedMode, setSelectedMode] = useState<'roulette' | 'level_filter'>('roulette');
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400">Assalomu alaykum, {user.username}! 👋</p>
          </div>
        ...
        {/* Online Users Banner */}
        <div className="bg-gradient-to-r from-emerald-600/20 to-primary-600/20 border border-emerald-500/30 rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-semibold">Hozir {onlineCount} ta foydalanuvchi online</p>
                <p className="text-slate-400 text-sm">Sherik topish oson!</p>
              </div>
            </div>
            {onlineCount > 1 && (
        ...
        <div className="card mb-8">
          <div className="flex items-center gap-6">
            ...
            <div className="text-right">
              <div className="text-sm text-slate-400">Hozirgi daraja</div>
              <div className="text-2xl font-bold text-primary-400">{formatLevel(user.current_level)}</div>
            </div>
            <div className="text-right">
              <div className="text-sm text-slate-400">Maqsad</div>
              <div className="text-2xl font-bold text-emerald-400">{formatLevel(user.target_score)}</div>
            </div>
          </div>
        </div>
        ...
        {queueStatus?.in_queue ? (
          ...
          <div className="card text-center">
                ...
                <div className="w-24 h-24 bg-primary-600/20 rounded-full flex items-center justify-center">
                  <div className="w-16 h-16 bg-primary-600/40 rounded-full flex items-center justify-center">
                    <div className="flex items-end gap-1 h-8">
                      <div className="w-1.5 bg-primary-500 rounded-full animate-pulse" style={{ height: '40%', animationDelay: '0ms' }} />
                      <div className="w-1.5 bg-primary-500 rounded-full animate-pulse" style={{ height: '70%', animationDelay: '150ms' }} />
                      <div className="w-1.5 bg-primary-500 rounded-full animate-pulse" style={{ height: '100%', animationDelay: '300ms' }} />
                      <div className="w-1.5 bg-primary-500 rounded-full animate-pulse" style={{ height: '60%', animationDelay: '450ms' }} />
                      <div className="w-1.5 bg-primary-500 rounded-full animate-pulse" style={{ height: '30%', animationDelay: '600ms' }} />
                    </div>
                  </div>
                </div>
            ...
            <p className="text-slate-400 mb-1">
              Rejim: <span className="text-primary-400 capitalize">{queueStatus.mode === 'roulette' ? 'Ruletka' : 'Daraja bo\'yicha'}</span>
            </p>
            <p className="text-slate-400 mb-1">
              Online: <span className="text-emerald-400">{onlineCount} ta</span>
            </p>
            <p className="text-slate-400 mb-6">
              Kutish vaqti: <span className="text-white font-mono">{waitTime} soniya</span>
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              ...
              <h3 className="text-xl font-semibold text-white mb-2">Ruletka rejimi</h3>
            ...
            >
              <div className="text-4xl mb-4">🎯</div>
              <h3 className="text-xl font-semibold text-white mb-2">Daraja bo'yicha</h3>
              <p className="text-slate-400 text-sm mb-4">
                O'zingizga yaqin darajadagi sherik bilan mashq qiling. 
        ...
        {!queueStatus?.in_queue && (
          <div className="mt-8 text-center">
            ...
            >
              {selectedMode === 'roulette' ? '🎲 Ruletka boshlash' : '🎯 Sherik topish'}
            </button>
            {!isConnected && (
        ...
        <div className="mt-8 card bg-slate-800/30">
          <h3 className="text-lg font-semibold text-white mb-4">Qanday ishlaydi?</h3>
